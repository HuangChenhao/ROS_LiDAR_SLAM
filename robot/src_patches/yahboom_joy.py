...
class JoyTeleop:
    def __init__(self):
        ...
        self.xspeed_limit = rospy.get_param("~xspeed_limit", 0.4)
        self.yspeed_limit = rospy.get_param('~yspeed_limit', 1.0)
        self.angular_speed_limit = rospy.get_param("~angular_speed_limit", 2.0)
        self.stable_drive = rospy.get_param('~stable_drive', False)
        self.stable_xspeed = rospy.get_param('~stable_xspeed', 0.35)
        self.stable_angular_speed = rospy.get_param('~stable_angular_speed', 2.2)
    ...
    def user_jetson(self, joy_data):
        ...
        # linear Gear control
        if False and joy_data.buttons[13] == 1:
        ...
        if False and joy_data.buttons[14] == 1:
        ...
        xlinear_speed = self.drive_axis(joy_data.axes[1], self.xspeed_limit, self.stable_xspeed) * self.linear_Gear
        ylinear_speed = 0.0
        angular_speed = self.drive_axis(joy_data.axes[2], self.angular_speed_limit, self.stable_angular_speed) * self.angular_Gear
    ...
    def user_pc(self, joy_data):
        ...
        # 档位控制 Gear control
        if False and joy_data.buttons[9] == 1:
            if self.linear_Gear == 1.0: self.linear_Gear = 1.0 / 3
            elif self.linear_Gear == 1.0 / 3: self.linear_Gear = 2.0 / 3
            elif self.linear_Gear == 2.0 / 3: self.linear_Gear = 1
        if False and joy_data.buttons[10] == 1:
            if self.angular_Gear == 1.0: self.angular_Gear = 1.0 / 4
            elif self.angular_Gear == 1.0 / 4: self.angular_Gear = 1.0 / 2
            elif self.angular_Gear == 1.0 / 2: self.angular_Gear = 3.0 / 4
            elif self.angular_Gear == 3.0 / 4: self.angular_Gear = 1.0
        xlinear_speed = self.drive_axis(joy_data.axes[1], self.xspeed_limit, self.stable_xspeed) * self.linear_Gear
        ylinear_speed = 0.0
        angular_speed = self.drive_axis(joy_data.axes[0], self.angular_speed_limit, self.stable_angular_speed) * self.angular_Gear
        if xlinear_speed > self.xspeed_limit: xlinear_speed = self.xspeed_limit
        elif xlinear_speed < -self.xspeed_limit: xlinear_speed = -self.xspeed_limit
        if ylinear_speed > self.yspeed_limit: ylinear_speed = self.yspeed_limit
        elif ylinear_speed < -self.yspeed_limit: ylinear_speed = -self.yspeed_limit
    ...
    def drive_axis(self, value, proportional_limit, stable_value):
        value = self.filter_data(value)
        if value == 0:
            return 0
        if not self.stable_drive:
            return value * proportional_limit
        if value > 0:
            return stable_value
        return -stable_value
