...
class yahboomcar_driver:
    def __init__(self):
        ...
        self.RA2DE = 180 / pi
        self.car = Rosmaster()
        self.car_type = os.environ.get("YAHBOOM_BASE_TYPE", "R2").upper()
        if self.car_type == "R2":
            self.car.set_car_type(self.car.CARTYPE_R2)
        else:
            self.car.set_car_type(self.car.CARTYPE_X3)
        ...
        self.cmd_vel_timeout = rospy.get_param('~cmd_vel_timeout', 0.35)
        self.last_cmd_time = time.time()
        # 速度反馈: 用 cmd_vel + IMU 代替坏掉的编码器
        ...
        self.watchdog_thread = threading.Thread(target=self.cmd_vel_watchdog)
    ...
    def pub_data(self):
        ...
        while not rospy.is_shutdown():
            ...
            # 原始编码器反馈(R2不工作): vx, vy, angular = self.car.get_motion_data()
            # 用 cmd_vel 命令速度作为线速度反馈
            vx = self.feedback_vx
            vy = 0.0
            angular = self.feedback_wz
            # 发布陀螺仪的数据
    ...
    def cmd_vel_callback(self, msg):
        ...
        self.timeout_stop_sent = False
        if self.car_type == "R2":
            # R2 Ackermann: map angular.z to steering range [-1.0, 1.0]
            # (original R2 driver sends linear.y directly as v_y up to +/-1.0)
            steer = max(min(angular / self.angular_limit, 1.0), -1.0)
            self.car.set_car_motion(vx * 0.9, steer, 0)
            self.feedback_vx = vx
            self.feedback_wz = angular
            return
